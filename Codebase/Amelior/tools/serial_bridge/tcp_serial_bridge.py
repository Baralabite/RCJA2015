...
# redirect config from a TCP/IP connection to a serial port and vice versa
...
class Redirector:
        ...
        while self.alive:
            try:
                ...
                if data:
                    ...
                    # escape outgoing config when needed (Telnet IAC (0xff) character)
    ...
    def write(self, data):
        """thread safe socket write with no config escaping. used to send telnet stuff"""
        self._write_lock.acquire()
        try:
...
""")
    ...
    parser.add_option("--spy",
        dest = "spy",
        action = "store_true",
        help = "peek at the communication and print all config to the console",
        default = False
    )
